...
wget --no-check-certificate -O /root/$qtPkg $qtUrl \
	...
	&& cd $qtTag \
	&& ./configure -nomake demos -nomake examples -fast -opensource -confirm-license \
                   -optimized-qmake -prefix /usr/lib64/qt4 -bindir /usr/lib64/qt4/bin \
				   -datadir /usr/lib64/qt4 -docdir /usr/share/doc/qt4 -headerdir /usr/include \
				   -libdir /usr/lib64 -plugindir /usr/lib64/qt4/plugins -sysconfdir /etc \
...
# manually creating symlinks only needed for CentOS 5
#case "$1" in
#	5)
